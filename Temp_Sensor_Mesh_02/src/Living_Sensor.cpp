...
#define WEBSOCKET_DISABLED 1

#define STASSID "Chillout"
#define STAPSK  "marlboro"
#define HOST_NAME "Living_Sen"
...
#define small_device_name "Mesh02" //6characters small name
...
float t = 0.0;
float h = 0.0;
constexpr char Topic[50] = "main/living/temperature/living";
...
void CheckForUpdates()
{
	...
	if(myhttpClient.connect("192.168.0.199",80))
	{
		...
		myhttpClient.println("GET /Mesh/versioning_m02 HTTP/1.1");
...
*/
	...
	JSONbuffer["Supply"] = (ESP.getVcc()/1000.0f)-0.2f; //get voltage in V instead of mV. Subtract adc offset of 200mV

	serializeJson(JSONbuffer, JSONmessageBuffer, sizeof(JSONmessageBuffer));
	debugA("%s",JSONmessageBuffer);
	MQTTclient.publish(Topic, JSONmessageBuffer, true);
